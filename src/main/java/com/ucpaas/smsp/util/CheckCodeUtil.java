...
//			g.setColor(color);
	...
	public static boolean check(HttpServletRequest request) {
		...
		String sessionCode = (String) request.getSession(true).getAttribute("randCheckCode");
		logger.debug("ip={}输入的验证码是：{}",CommonUtil.getClientIP(request),input);
		logger.debug("ip={}的session中验证码是：{}",CommonUtil.getClientIP(request),sessionCode);
		if (StringUtils.isNotBlank(sessionCode) && StringUtils.isNotBlank(input) && sessionCode.trim().equalsIgnoreCase(input.trim())) {
			return true;
		}
		return false;
	}
